...
const getAll = async (req, res) =>{
    ...
    const searchFields = ['`code`', '`product`.`name`', '`auction`.`name`', '`status`.`name`'];

    const inclusions = [
        { model: ProductsModel, as: 'product' },
        { model: AuctionsModel, as: 'auction' },
        { model: StatusModel, as: 'status' },
    ];

    if(req.query.auctionId > 0){
        req.query.extraFields = [
            ['auction_id', req.query.auctionId, Sequelize.Op.eq]
        ]
    }
...
const save = async (req, res) =>{
    try {
        ...
        const auction = await AuctionsModel.findByPk(lotData.auction_id);

        // replic start and end time from Auction to lot
        lotData.start = auction.start;
        lotData.end = auction.end;

        const lot = await LotsModel.create(lotData, { fields });
        resultSuccess('Dados salvos com sucesso.', res)(lot);
    } catch (error) {
